...
namespace Common.Utilities.Formatter
{
  public class RecordFormatter : IRecordFormatter
  {
    ...
    public IEnumerable<string> FormatFile(string filePath, string recordDelimiter, bool removeBlankRecords)
    {
      ...
      return FormatRecord(fileContent, recordDelimiter, removeBlankRecords);
    }
    ...
    {
      return records.Where(r => { return removeBlankRecords ? string.IsNullOrWhiteSpace(r) == false : true; }).Select(r => FormatRecord(r, subRecordDelimiter, removeBlankRecords));
    }
  }
}
